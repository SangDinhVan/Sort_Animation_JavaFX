...
public class ShellSort implements DataReceiver {
    ...
    public void swapFields(int index1, int index2, Runnable callback) {
        ...
        first.setStyle("-fx-background-color: #FF6347;");  
        second.setStyle("-fx-background-color: #ADD8E6;"); 
        ...
        parallelTransition.setOnFinished(e -> {
            ...
            if (isCorrectPosition(index1)) {
                first.setStyle("-fx-background-color: #80EE98;"); 
            } else {
                first.setStyle("-fx-background-color: #F0F0F0;"); 
            }

            if (isCorrectPosition(index2)) {
                second.setStyle("-fx-background-color: #80EE98;");
            } else {
    ...
    private void shellSortStep() {
        if (gap > 0) {
            if (i < values.length) {
                int temp = values[i];
                j = i;
                if (j >= gap && values[j - gap] > temp) {
                    swapFields(j, (j - gap), () -> {
                        i--;
                        shellSortStep();
                    });
